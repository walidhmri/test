      ...
      <div class="col-12">
        <div class="card my-4">
          ...
          <div class="card-body px-4 pb-2">
            <form id="ticketForm" method="post" action="{{  route('employee.tickets.create')}}">
              @csrf
              <div class="mb-3">
                <label class="form-label">Titre</label>
                <input type="text" name="title" class="form-control" id="ticketTitle" placeholder="Entrez le titre du ticket" required>
              </div>
              <div class="mb-3">
                <label class="form-label">Description</label>
                <textarea name="description" class="form-control" id="ticketDescription" rows="3" placeholder="Décrivez le problème" required></textarea>
              </div>
              <div class="mb-3">
                <label class="form-label">Priorité</label>
                <select  name="priority" class="form-control" id="ticketPriority" required>
                  <option  value="">Sélectionnez une priorité</option>
                  <option name="" value="low">Basse</option>
                  <option value="medium">Moyenne</option>
                  <option value="high">Haute</option>
                  <option value="urgent">Urgente</option>
                </select>
              </div>
              <button type="submit" class="btn bg-gradient-dark">Envoyer le ticket</button>
            </form>
          </div>
        </div>
      </div>
